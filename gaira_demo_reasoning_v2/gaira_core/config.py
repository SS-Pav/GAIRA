...
# ─────────────────────────────────────────────────────────────────────
# Paths (v2: migration-hardened — resolved via gaira_core/paths.py)
#
# All roots below are resolved through paths.py so the demo is portable:
#   * env overrides GAIRA_DATA_ROOT / GAIRA_LEGACY_DEMO_DATA win
#   * candidate mount/home locations are probed
#   * the tiny legacy CSVs are bundled inside v2 (self-contained)
#   * a missing external volume yields a path that simply won't exist, and
#     every loader already falls back to a labelled placeholder.
# Constant NAMES are kept identical to v1 so data_loader.py is unchanged.
# ─────────────────────────────────────────────────────────────────────

DEMO_ROOT = _paths.DEMO_ROOT
...
GENERATED_DIR = DATA_ROOT / "generated"   # v2: derived, committed evidence tables

GAIRA_REPO_ROOT = _paths.REPO_ROOT

# Legacy demo CSVs — never None (bundled fallback inside v2/data/legacy).
LEGACY_DEMO_DATA = _paths.legacy_demo_data()

# External processed-data volume (GAIRA autoresearch outputs). When the volume
# is not resolvable, GAIRA_DATA_VOLUME points at a best-guess candidate that
# won't exist — loaders degrade to placeholders and the app banner says so.
GAIRA_DATA_VOLUME = _paths.data_root() or Path("/Volumes/SSD_Rad/GAIRA_DATA")
AUTORESEARCH_ROOT = (_paths.autoresearch_root()
                       or (GAIRA_DATA_VOLUME
