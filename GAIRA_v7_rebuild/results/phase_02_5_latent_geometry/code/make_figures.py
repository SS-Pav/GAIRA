...
def save(fig, name):
    ...
    fig.savefig(F / f"{name}.png", dpi=190)
    plt.close(fig)
    print(f"  {name}")
...
def f14_community_stability(c):
    ...
    fig.suptitle("Cluster quality across K and linkage — the three indices do not agree, "
                 "which is itself the finding: no K is a real cluster count",
