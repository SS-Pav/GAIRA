...
def main() -> None:
    ...
    output_dir = processed_root / "hcc_serum_class_reference_matches"
    output_dir.mkdir(parents=True, exist_ok=True)

    class_summaries_df = load_class_summaries(db_path)
    if class_summaries_df.empty:
        print("No hcc_serum class summaries were found. Run process_biosample_dataset.py first.")
        return
    ...
    for row in class_summaries_df.to_dict(orient="records"):
        ...
        _, candidate_df_reranked = engine.rerank_with_spectral_similarity(
            normalized_df,
            candidate_df_ref,
        )
