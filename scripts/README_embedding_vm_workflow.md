...
1. Build the training dataset locally from the SSD-backed DuckDB source of truth:
   ```bash
   PYTHONPATH=src .venv/bin/python scripts/build_embedding_dataset.py --output-dir /Volumes/SSD_Rad/GAIRA_DATA/processed/embedding_v2
   ```
...
3. Sync training-ready artifacts to the VM:
   ```bash
   scripts/sync_embedding_artifacts_to_vm.sh /Volumes/SSD_Rad/GAIRA_DATA/processed/embedding_v2 ~/projects/GAIRA/data/processed/embedding_v2
   ```

4. Run training and evaluation on the VM:
   ```bash
   EPOCHS=30 scripts/run_embedding_training_vm.sh
   ```
