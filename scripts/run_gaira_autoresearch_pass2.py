...
from gaira.demo.autoresearch_pass2_utils import (
    Pass2HarnessConfig,
    build_pass2_markdown_report,
    build_pass2_search_space,
    build_pdf_report,
    compute_pass2_outputs,
    plot_pass2_best_vs_baseline,
    plot_pass2_factor_ablation,
    plot_pass2_leaderboards,
    save_pass2_summary_tables,
    score_pass2_outputs,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run GAIRAv3 autoresearch v1 pass 2 chemistry-focused calibration.")
    parser.add_argument("--storage-config-path", type=Path, default=DEFAULT_STORAGE_CONFIG_PATH)
    parser.add_argument("--sprint-subdir", type=str, default="pass2_chemistry_focus")
...
def main() -> None:
    ...
    search_df = build_pass2_search_space()
    results_rows = []
    ...
    summary_tables = save_pass2_summary_tables(sprint_paths, search_df, results_df)
    figure_paths = []
    figure_paths.extend(plot_pass2_leaderboards(completed_df, sprint_paths).values())
    figure_paths.append(plot_pass2_factor_ablation(completed_df, sprint_paths))
    figure_paths.append(
        plot_pass2_best_vs_baseline(
            completed_df,
            sprint_paths,
            "cspp_metabolite_spike_validation",
            "pass2_best_vs_baseline_cspp.png",
        )
    )
    figure_paths.append(
        plot_pass2_best_vs_baseline(
            ...
            "pass2_best_vs_baseline_uricase.png",
        )
    )
    report_md = build_pass2_markdown_report(sprint_paths, completed_df, search_df, summary_tables["best"])
    build_pdf_report(report_md, figure_paths, sprint_paths.report_dir / "GAIRA_autoresearch_v1_pass2_report.pdf")
    print(f"Wrote pass 2 autoresearch outputs under {sprint_paths.sprint_root}")
