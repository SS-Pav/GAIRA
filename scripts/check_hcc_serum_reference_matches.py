...
def main() -> None:
    ...
    matches_dir = processed_root / "hcc_serum_class_reference_matches"
    ...
    top_molecules_df = (
        pd.DataFrame([{"component": key, "count": value} for key, value in molecule_counter.items()])
        .sort_values(["count", "component"], ascending=[False, True])
        .reset_index(drop=True)
    )
    top_classes_df = (
        pd.DataFrame([{"biochemical_class": key, "count": value} for key, value in class_counter.items()])
