...
def _prepare_fullspectra_query_df(query_df: pd.DataFrame) -> pd.DataFrame:
    work = query_df.reset_index(drop=True).copy()
    work["sample_id"] = work.apply(_extract_sample_id, axis=1)
    work["broad_class_label"] = work["class_label"].astype(str).map(_parse_day)
    ...
    work["trajectory_index"] = [
        ...
        for day_label, concentration in zip(
            work["broad_class_label"].astype(str),
...
def _aggregate_sample_level_bsv(per_spectrum_df: pd.DataFrame, axes: list[str]) -> pd.DataFrame:
    sample_level = (
        per_spectrum_df.groupby(
            ["sample_id", "class_label", "broad_class_label", "trajectory_concentration", "trajectory_index"],
            as_index=False,
        )[axes]
        .mean()
    )
    sample_level["sample_key"] = "sample_mean__" + sample_level["sample_id"].astype(str)
    return sample_level[["sample_key", "sample_id", "class_label", "broad_class_label", "trajectory_concentration", "trajectory_index"] + axes]


def _aggregate_sample_level_family(spectrum_family_df: pd.DataFrame) -> pd.DataFrame:
    grouped = (
        spectrum_family_df.groupby(
            ["sample_id", "class_label", "broad_class_label", "trajectory_concentration", "trajectory_index", "family"],
            as_index=False,
        )["family_fraction"]
        .mean()
    )
    grouped["sample_key"] = "sample_mean__" + grouped["sample_id"].astype(str)
    return grouped[["sample_key", "sample_id", "class_label", "broad_class_label", "trajectory_concentration", "trajectory_index", "family", "family_fraction"]]
...
def _build_verification_outputs(
    ...
    fullspectra_query_df: pd.DataFrame,
    tables_dir: Path,
    report_dir: Path,
) -> pd.DataFrame:
        ...
        [
            {
                ...
                "subset_alias": SUBSET_ALIAS,
                "raw_query_rows": int(len(raw_query_df)),
                "raw_unique_sample_ids": int(fullspectra_query_df["sample_id"].astype(str).nunique()),
                "raw_unique_biosample_ids": int(len(raw_query_df)),
                "rows_passed_to_bsv_generation_previous_pilot3": int(fullspectra_query_df["sample_id"].astype(str).nunique()),
                "rows_passed_to_bsv_generation_fullspectra": int(len(fullspectra_query_df)),
                "previous_pilot3_used_sample_means": True,
                "full_spectra_available_locally": True,
                "aggregation_code_path": "scripts/run_gaira_pilot3_shine_ev_sers.py::_build_sample_query_df -> groupby('derived_sample_id') -> mean spectrum -> sample_mean__* passed to build_bsv_profiles_pass5",
            }
        ]
    )
    verification_df.to_csv(tables_dir / "pilot3_fullspectra_verification.csv", index=False)
    note_lines = [
        "# Pilot 3 Full-Spectra Verification Note",
        "",
        "- Previous Pilot 3 did use sample means: `yes`.",
        f"- Raw SHINE query dataframe rows available locally: `{len(raw_query_df)}`.",
        f"- Unique derived sample IDs in the local SHINE subset: `{fullspectra_query_df['sample_id'].astype(str).nunique()}`.",
        f"- Previous Pilot 3 rows passed into BSV generation: `{fullspectra_query_df['sample_id'].astype(str).nunique()}`.",
        f"- Full-spectrum rows passed into corrected BSV generation: `{len(fullspectra_query_df)}`.",
        "- Full spectra are available locally: `yes`.",
        "- Exact aggregation path in the previous script:",
        "  - `load_query_dataframe(...)` returned the full per-spectrum SHINE subset.",
        "  - `_build_sample_query_df(...)` called `decode_and_align(...)`, grouped by `derived_sample_id`, averaged the aligned spectra, and created `sample_mean__<sample_id>` records.",
        "  - `build_bsv_profiles_pass5(...)` was then run on those aggregated sample-mean rows rather than the original per-spectrum dataframe.",
        "",
    ]
    (report_dir / "pilot3_fullspectra_verification_note.md").write_text("\n".join(note_lines), encoding="utf-8")
...
def _build_report(
    ...
    previous_class = pd.read_csv(PREVIOUS_PILOT3_ROOT / "tables" / "class_level_metrics.csv")
    previous_cluster = pd.read_csv(PREVIOUS_PILOT3_ROOT / "tables" / "cluster_selection_summary.csv")
    previous_corr = pd.read_csv(PREVIOUS_PILOT3_ROOT / "tables" / "trajectory_correlation_metrics.csv")
    previous_smooth = pd.read_csv(PREVIOUS_PILOT3_ROOT / "tables" / "trajectory_smoothness_metrics.csv")
    previous_best = previous_cluster.sort_values("selection_score", ascending=False).groupby("class_label", as_index=False).first()
    previous_top_corr = previous_corr.iloc[previous_corr["spearman_r"].abs().fillna(0.0).argmax()]
    lines = [
        "# GAIRAv3 Pilot 3 SHINE EV SERS Full-Spectra Report",
        "",
        "## 1. Verification Result",
        "- The previous Pilot 3 used sample means before BSV generation.",
        "- The local repo does contain the full SHINE per-spectrum dataset and it was used in this rerun.",
        f"- Full query rows used in the corrected rerun: `{metadata_summary['n_fullspectra_rows']}`.",
        f"- Derived biological sample IDs present locally: `{metadata_summary['n_sample_ids']}`.",
        "- The corrected script keeps per-spectrum BSV as the primary representation and derives sample-level summaries only after the full-spectrum pass.",
        "",
        "## 2. Dataset Overview",
        ...
        "",
        "## 7. Comparison to Previous Sample-Mean Pilot 3",
        f"- Previous sample-mean broad-label silhouette: `{float(previous_class['silhouette_score_delta_bsv'].iloc[0]):.4f}`; full-spectrum broad-label silhouette: `{broad_sil:.4f}`.",
        f"- Previous sample-mean mean cluster silhouette: `{float(previous_best['mean_silhouette'].mean()):.4f}`; full-spectrum mean cluster silhouette: `{mean_cluster_sil:.4f}`.",
        f"- Previous sample-mean strongest trajectory Spearman: `{float(previous_top_corr['spearman_r']) if pd.notna(previous_top_corr['spearman_r']) else 0.0:.4f}`; full-spectrum strongest trajectory Spearman: `{float(top_corr['spearman_r']) if pd.notna(top_corr['spearman_r']) else 0.0:.4f}`.",
        f"- Previous sample-mean mean adjacent condition distance: `{float(previous_smooth['adjacent_distance_delta_bsv'].mean()):.4f}`; full-spectrum mean adjacent condition distance: `{mean_adjacent:.4f}`.",
        "- The scientific question is whether the earlier conclusions survive once the representation is run on all available spectra instead of on sample averages.",
        "",
        "## 8. Interpretation",
        ...
        "- The corrected full-spectrum SHINE run should now be treated as the trustworthy Pilot 3 result.",
        "- Broad-label day separation remains weak.",
        "- Latent-state discovery remains strong and stable.",
        "- Trajectory signal remains weak to modest rather than strongly monotonic.",
        "- The scientific conclusion does not flip: cfg05 behaves as a stable latent-state representation more than as a strong top-level label or progression separator on SHINE.",
        "",
    ]
    report_path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def _build_samplemean_vs_fullspectra_comparison(
    current_class_metrics_df: pd.DataFrame,
    current_cluster_selection_df: pd.DataFrame,
    current_trajectory_corr_df: pd.DataFrame,
    current_trajectory_smooth_df: pd.DataFrame,
) -> pd.DataFrame:
    previous_class = pd.read_csv(PREVIOUS_PILOT3_ROOT / "tables" / "class_level_metrics.csv")
    previous_cluster = pd.read_csv(PREVIOUS_PILOT3_ROOT / "tables" / "cluster_selection_summary.csv")
    previous_corr = pd.read_csv(PREVIOUS_PILOT3_ROOT / "tables" / "trajectory_correlation_metrics.csv")
    ...
    def summarize(label: str, class_df: pd.DataFrame, cluster_df: pd.DataFrame, corr_df: pd.DataFrame, smooth_df: pd.DataFrame) -> dict[str, object]:
        best_cluster = cluster_df.sort_values("selection_score", ascending=False).groupby("class_label").first().reset_index()
        top_corr = corr_df.iloc[corr_df["spearman_r"].abs().fillna(0.0).argmax()]
        return {
            "run_type": label,
            "broad_label_silhouette": float(class_df["silhouette_score_delta_bsv"].iloc[0]),
            "between_class_centroid_distance": float(class_df["between_class_distance_delta_bsv"].mean()),
            "within_class_variance": float(class_df["within_class_variance_delta_bsv"].mean()),
            "selected_cluster_k": "; ".join(f"{row.class_label}={int(row.k)}" for row in best_cluster.itertuples(index=False)),
            "cluster_silhouette": float(best_cluster["mean_silhouette"].mean()),
            "cluster_stability": float(best_cluster["mean_ari"].mean()),
            "strongest_trajectory_metric": f"{top_corr['trajectory_variable']}::{top_corr['metric_name']}",
            "strongest_trajectory_spearman": float(top_corr["spearman_r"]) if pd.notna(top_corr["spearman_r"]) else 0.0,
            "mean_adjacent_condition_distance": float(smooth_df["adjacent_distance_delta_bsv"].mean()),
        }

    return pd.DataFrame(
        [
            summarize("sample_mean_previous_pilot3", previous_class, previous_cluster, previous_corr, previous_smooth),
            summarize("full_spectra_pilot3", current_class_metrics_df, current_cluster_selection_df, current_trajectory_corr_df, current_trajectory_smooth_df),
        ]
    )
...
def main() -> None:
    ...
    fullspectra_query_df = _prepare_fullspectra_query_df(query_df)
    _build_verification_outputs(query_df, fullspectra_query_df, sprint_paths.tables_dir, sprint_paths.report_dir)

    grounding_df, mapping_df, harness_config, unavailable_sources = _prepare_grounding_and_mapping(
        registries, resolved, CONFIG_SPEC
    )
    spectrum_bsv_df, spectrum_retrieval_df = build_bsv_profiles_pass5(
        fullspectra_query_df,
        grounding_df,
        mapping_df,
    ...
    spectrum_bsv_df = spectrum_bsv_df.merge(
        fullspectra_query_df[
            [
                "sample_key",
                "sample_id",
    ...
    spectrum_delta_df = _cohort_delta(spectrum_bsv_df[["sample_key"] + axes].copy(), axes).merge(
        spectrum_bsv_df[
            [
                "sample_key",
                "sample_id",
    ...
    broad_metrics_df, broad_family_df, broad_pairwise_delta_df = _broad_class_metrics(
        ...
        spectrum_delta_df,
        pd.DataFrame(),
        spectrum_retrieval_df,
        axes,
    )
    broad_class_mean_bsv_df = spectrum_bsv_df.groupby("broad_class_label", as_index=False)[axes].mean().rename(columns={"broad_class_label": "class_label"})
    broad_class_mean_delta_df = spectrum_delta_df.groupby("broad_class_label", as_index=False)[axes].mean().rename(columns={"broad_class_label": "class_label"})

    condition_mean_bsv_df = spectrum_bsv_df.groupby("class_label", as_index=False)[axes].mean()
    condition_mean_delta_df = spectrum_delta_df.groupby("class_label", as_index=False)[axes].mean()
    condition_pairwise_delta_df = pairwise_delta_bsv(condition_mean_delta_df, axes)

    class_neighborhood_df = build_class_topk_neighborhood_composition(spectrum_retrieval_df)
    spectrum_family_df = _sample_family_fingerprint(spectrum_retrieval_df, fullspectra_query_df)
    sample_bsv_df = _aggregate_sample_level_bsv(spectrum_bsv_df, axes)
    sample_delta_df = _cohort_delta(sample_bsv_df[["sample_key"] + axes].copy(), axes).merge(
        sample_bsv_df[["sample_key", "sample_id", "class_label", "broad_class_label", "trajectory_concentration", "trajectory_index"]],
        on="sample_key",
        how="left",
    )
    sample_family_df = _aggregate_sample_level_family(spectrum_family_df)
    broad_family_df = spectrum_family_df.groupby(["broad_class_label", "family"], as_index=False)["family_fraction"].mean().rename(
        columns={"broad_class_label": "class_label"}
    )

    sample_meta = fullspectra_query_df[["sample_key", "sample_id", "class_label", "broad_class_label", "trajectory_concentration", "trajectory_index"]].copy()
    spectral_scores, spectral_explained = _fit_pca(np.vstack([np.array(json.loads(x), dtype=float) for x in fullspectra_query_df["intensity_json"]]), scale=True)
    ...
    bsv_pca_df = _pca_dataframe(spectrum_bsv_df, axes, scale=True)
    delta_pca_df = _pca_dataframe(spectrum_delta_df, axes, scale=True)
    umap_bsv_df = _maybe_umap_dataframe(spectrum_bsv_df, axes)
    umap_delta_df = _maybe_umap_dataframe(spectrum_delta_df, axes)

    clustered_parts = []
    cluster_selection_parts = []
    for broad_class_label in DAY_ORDER:
        if broad_class_label not in set(spectrum_delta_df["broad_class_label"].astype(str)):
            continue
        clustered_sub, selection_sub = _cluster_with_stability(spectrum_delta_df, axes, broad_class_label)
    ...
    spectrum_bsv_df.to_csv(tables_dir / "per_spectrum_bsv.csv", index=False)
    spectrum_delta_df.to_csv(tables_dir / "per_spectrum_delta_bsv.csv", index=False)
    ...
    _build_samplemean_vs_fullspectra_comparison(
        broad_metrics_df,
        cluster_selection_df,
        trajectory_corr_df,
        trajectory_smooth_df,
    ).to_csv(tables_dir / "pilot3_samplemean_vs_fullspectra_comparison.csv", index=False)

    _plot_scatter(spectral_pca_df, "pc1", "pc2", figures_dir / "pca_spectral_broad_classes_fullspectra.png", title="Spectral PCA of SHINE Full Spectra", hue_col="broad_class_label")
    _plot_scatter(bsv_pca_df, "pc1", "pc2", figures_dir / "pca_bsv_broad_classes_fullspectra.png", title="BSV PCA by Broad Day Class (Full Spectra)", hue_col="broad_class_label")
    _plot_scatter(delta_pca_df, "pc1", "pc2", figures_dir / "pca_delta_bsv_broad_classes_fullspectra.png", title="Delta-BSV PCA by Broad Day Class (Full Spectra)", hue_col="broad_class_label")
    if umap_bsv_df is not None:
        _plot_scatter(umap_bsv_df.rename(columns={"u1": "pc1", "u2": "pc2"}), "pc1", "pc2", figures_dir / "umap_bsv_broad_classes_fullspectra.png", title="BSV UMAP by Broad Day Class (Full Spectra)", hue_col="broad_class_label")
    if umap_delta_df is not None:
        _plot_scatter(umap_delta_df.rename(columns={"u1": "pc1", "u2": "pc2"}), "pc1", "pc2", figures_dir / "umap_delta_bsv_broad_classes_fullspectra.png", title="Delta-BSV UMAP by Broad Day Class (Full Spectra)", hue_col="broad_class_label")
    _plot_radar_grid(broad_class_mean_bsv_df, "class_label", figures_dir / "radar_bsv_broad_classes_fullspectra.png", "Broad-Day Absolute BSV Fingerprints (Full Spectra)", delta_mode=False)
    _plot_radar_grid(broad_class_mean_delta_df, "class_label", figures_dir / "radar_delta_bsv_broad_classes_fullspectra.png", "Broad-Day Delta-BSV Fingerprints (Full Spectra)", delta_mode=True)
    _plot_family_bars(broad_family_df, "class_label", figures_dir / "class_family_fingerprint_bars_fullspectra.png", "Broad-Day Neighborhood Family Composition (Full Spectra)")
    ...
    _plot_radar_grid(cluster_abs_df.rename(columns={"cluster_label": "cluster_label"}), "cluster_label", figures_dir / "cluster_radar_absolute_fullspectra.png", "Cluster Absolute BSV Fingerprints (Full Spectra)", delta_mode=False)
    _plot_radar_grid(cluster_delta_df.rename(columns={"cluster_label": "cluster_label"}), "cluster_label", figures_dir / "cluster_radar_delta_fullspectra.png", "Cluster Delta-BSV Fingerprints (Full Spectra)", delta_mode=True)
    ...
    _plot_family_radar_grid(cluster_family_wide[["cluster_label"] + FAMILY_ORDER], "cluster_label", figures_dir / "cluster_radar_family_fullspectra.png", "Cluster Family Composition Fingerprints (Full Spectra)")
    ...
    if not cluster_alignment_df.empty:
        ...
        plt.close(fig)
    _plot_cross_class_overlap(cross_class_df, figures_dir / "cross_class_overlap_heatmap_fullspectra.png")

    _plot_trajectory_pca(bsv_pca_df, figures_dir / "trajectory_pca_bsv.png", title="Trajectory Overlay in BSV PCA Space (Full Spectra)")
    _plot_trajectory_pca(delta_pca_df, figures_dir / "trajectory_pca_delta_bsv.png", title="Trajectory Overlay in Delta-BSV PCA Space (Full Spectra)")
    _plot_trajectory_cluster_distribution(trajectory_cluster_df, figures_dir / "trajectory_cluster_distribution_fullspectra.png")
    _plot_trajectory_metric_trends(spectrum_delta_df, delta_pca_df, spectrum_family_df, figures_dir / "trajectory_metric_trends_fullspectra.png")

    metadata_summary = {
        "subset_alias": SUBSET_ALIAS,
        "n_fullspectra_rows": int(len(fullspectra_query_df)),
        "n_sample_ids": int(fullspectra_query_df["sample_id"].astype(str).nunique()),
        "class_labels": sorted(fullspectra_query_df["class_label"].astype(str).unique().tolist()),
        "subclass_labels": sorted(fullspectra_query_df["subclass_label"].astype(str).unique().tolist()),
        "day_values": DAY_ORDER,
        "concentration_values": sorted(fullspectra_query_df["trajectory_concentration"].astype(int).unique().tolist()),
    }
    report_md = report_dir / "GAIRAv3_Pilot3_shine_ev_sers_fullspectra_report.md"
    report_pdf = report_dir / "GAIRAv3_Pilot3_shine_ev_sers_fullspectra_report.pdf"
    _build_report(
        report_md,
        metadata_summary,
        broad_metrics_df,
