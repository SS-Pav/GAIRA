...
def main() -> None:
    ...
    report_path = processed_root / "shine_class_reference_matches" / "shine_class_explanation_report_v5.csv"
    retrieval_path = processed_root / "shine_class_reference_matches" / "shine_class_supporting_chunks_v5.csv"

    if not report_path.exists():
        print(f"Explanation report v5 not found: {report_path}")
        return
    if not retrieval_path.exists():
        print(f"Supporting chunks v5 not found: {retrieval_path}")
        return
    ...
    confounder_dominant_rows = 0
    for _, group_df in retrieval_df.groupby(["class_label", "subclass_label"]):
        roles = group_df["chunk_role"].astype(str).tolist()
        if roles.count("confounder_or_caution") >= max(1, len(roles) - 1):
            confounder_dominant_rows += 1

    print(f"Explanation report v5 rows: {len(report_df)}")
    ...
    print(f"\nConfounder-dominant rows: {confounder_dominant_rows}")
    print("\nFirst 10 rows:")
    print(
        report_df[
            [
