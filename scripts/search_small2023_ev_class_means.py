...
def load_class_summaries(db_path: Path) -> pd.DataFrame:
    """Load processed class-mean spectra from DuckDB."""
...
def main() -> None:
    ...
    output_dir = processed_root / "small2023_ev_class_reference_matches"
    output_dir.mkdir(parents=True, exist_ok=True)

    class_summaries_df = load_class_summaries(db_path)
    if class_summaries_df.empty:
        print("No small2023_ev class summaries were found. Run process_biosample_dataset.py first.")
        return
