...
Canonicalised so that a registry written on two machines hashes identically: float64
C-contiguous arrays, JSON with sorted keys and no insignificant whitespace, CSV with fixed
column order and LF line endings.
"""
...
from .motif import ACS
from .registry import ACSRegistry

SERIALIZATION_VERSION = "v7_acs_serialization_v1"
...
def registry_fingerprint(registry: ACSRegistry) -> str:
    ...
    so the hash is stable across runs and machines.
    """
...
def save_registry(registry: ACSRegistry, out_dir) -> dict:
    ...
    p = out_dir / "acs_manifest_v1.json"
    p.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n")
    return manifest


def load_registry(out_dir) -> tuple[pd.DataFrame, np.ndarray, list[str], dict]:
    """Round-trip read. Returns (motif table, spectra, motif ids, manifest)."""
    out_dir = Path(out_dir)
    df = pd.read_csv(out_dir / "acs_registry_v1.csv")
    z = np.load(out_dir / "acs_spectra_v1.npz", allow_pickle=True)
    manifest = json.loads((out_dir / "acs_manifest_v1.json").read_text())
...
def motifs_from_table(df: pd.DataFrame, spectra: np.ndarray, ids: list[str]) -> list[ACS]:
    """Rebuild retained ACS objects from a serialised registry (round-trip check)."""
    ...
    for _, r in df[df.retained].iterrows():
        out.append(ACS(
            ...
            rejection_reason=("" if pd.isna(r.rejection_reason)
