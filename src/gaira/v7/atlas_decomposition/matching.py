"""GAIRA V7 — matching spectra and analytes to Atlas Component Substructures.
...
The motif layer only ever *redistributes* an activation the frozen atlas already produced.
Summed over the motifs of component k, the attributed evidence equals w_k exactly, so no
new evidence is created and the atlas remains the sole source of activation.
"""
...
def band_mass(x: np.ndarray, motif: ACS, half_width: int = 4) -> np.ndarray:
...
def motif_affinity(x: np.ndarray, motif: ACS, half_width: int = 4) -> float:
...
def attribute_component(x: np.ndarray, activation: float, motifs: list[ACS],
                        half_width: int = 4) -> dict[str, float]:
    """Split one component's activation across its motifs, conserving the total.

    Returns {motif_id: attributed activation}, summing to `activation`. With no motifs, or
    no affinity at all, the activation is returned under the reserved key `_unattributed`
    rather than being silently dropped — evidence must never disappear.
    """
    if not motifs or activation <= 0:
        return {UNATTRIBUTED: float(max(activation, 0.0))}
    aff = np.array([motif_affinity(x, m, half_width) for m in motifs], float)
    if aff.sum() <= 0:
        return {UNATTRIBUTED: float(activation)}
    w = aff / aff.sum()
...
def attribution_matrix(X: np.ndarray, W: np.ndarray, registry,
    ...
    ids = [m.motif_id for m in registry.retained] + [UNATTRIBUTED]
