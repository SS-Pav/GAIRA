"""GAIRA V7 — the Local Spectral Motif (LSM) object.

WHAT AN LSM IS IN PHASE 01
--------------------------
An LSM is a **deterministic decomposition of one frozen atlas component into a reusable
spectral substructure**. It is NOT a new atlas component and it never enters projection.
    ...
    frozen atlas component  h_k  ∈ ℝ₊^676        (24 of these, immutable)
              │
              ├── LSM k.0    a subset of h_k's bands that co-occur across a coherent
              ├── LSM k.1    group of analytes activating component k
              └── …
...
----------------------------------------------------------------------------------
`GAIRA_v7_rebuild/architecture/LEARNING_MODE_ARCHITECTURE.md` defines an LSM as a row of
...
"local spectral motifs" in the sense of local substructure, but they are not the same
...
@dataclass
class LSM:
    """A Local Spectral Motif: a substructure of one frozen atlas component."""
