"""GAIRA V7 — Phase 01 regression tests: the Local Spectral Motif layer.
...
from gaira.v7.lsm.motif import LSM, Band, build_motif_spectrum   # noqa: E402
...
def test_lsm_validate_catches_violations(toy):
    h, _, bands, _ = toy
    good = LSM(motif_id="c00.m00", parent_component=0, index_in_component=0,
    ...
    bad = LSM(**{**good.__dict__, "spectrum": -good.spectrum})
    assert any("negative" in v for v in bad.validate())

    bad = LSM(**{**good.__dict__, "n_analytes": 99})
    assert any("n_analytes" in v for v in bad.validate())

    bad = LSM(**{**good.__dict__, "retained": False})
...
def _mk(mid, n_analytes, stability=1.0, n_bands=3, spectrum=None):
    ...
    return LSM(motif_id=mid, parent_component=0, index_in_component=int(mid.split("m")[1]),
...
def test_attribution_is_batch_independent(toy):
    ...
    for m, bs in zip(ms, ([0, 1], [2, 3])):
        m.band_indices = [bands[i].index for i in bs]
        m.band_weights = [0.5, 0.5]
    reg = LSMRegistry([{"component": 0, "status": "DECOMPOSED", "n_bands": 4,
...
def test_registry_keeps_rejected_motifs_queryable():
    ...
    reg = LSMRegistry([{"component": 0, "status": "IRREDUCIBLE", "n_bands": 4,
...
def test_serialization_round_trips(tmp_path):
    ...
    reg = LSMRegistry([{"component": 0, "status": "DECOMPOSED", "n_bands": 2,
...
def test_phase00_artifacts_untouched():
    ...
    assert not stale, f"Phase 00 artefacts modified by Phase 01: {stale}"


@ran
def test_phase01_writes_only_inside_its_own_tree():
    man = json.loads((A / "phase_01_manifest_v1.json").read_text())
    stray = [o["path"] for o in man["outputs"]
             if not o["path"].startswith("results/v7_rebuild/phase01/")]
    assert not stray, f"Phase 01 wrote outside its tree: {stray}"
...
def test_committed_registry_is_intact():
    df = pd.read_csv(T / "lsm_registry_v1.csv")
...
def test_committed_motif_spectra_are_non_negative_and_gridded():
    z = np.load(A / "lsm_spectra_v1.npz", allow_pickle=True)
...
@ran
def test_all_phase01_gates_pass():
    state = json.loads((P01 / "PHASE_STATE.json").read_text())
    failed = [g["gate"] for g in state["gates"] if not g["passed"]]
    assert not failed, f"Phase 01 gates failed: {failed}"
...
@ran
def test_no_phase02_work_leaked_in():
