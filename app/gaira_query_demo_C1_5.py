"""GAIRA Query Demo v1.5 — coverage-aware, normalized, stability-checked.

Run: streamlit run app/gaira_query_demo_C15.py
"""
...
st.set_page_config(page_title="GAIRA v1.5", page_icon="🔬", layout="wide")
st.title("GAIRA — Graph-Grounded Spectral Interpretation v1.5")
st.caption(f"Version {VERSION} | 1,887 evidence rows | 137 sources | 181 motifs | Coverage-aware comparative scoring")
...
with st.sidebar:
    ...
    **Enrichment:** `Enriched in HCC vs rest?`
    **Peak:** `Peak 1005?`
    **Theme:** `Lipid signal?`
    **Chemistry:** `Amide I to biology?`
    """)
    st.caption("v1.5: normalized enrichment, zero-comparator fixes, evidence balance.")
...
if scored.comparator_summary:
    ...
    st.caption(f"Sample type: **{cs.inferred_sample_type}** | Evidence balance: **{cs.evidence_balance:.2f}**")
...
def _tdf(themes, show_comp=False):
    ...
    for t in themes:
        ...
        if show_comp:
            ...
            r["Bal."] = f"{t.get('evidence_balance',0):.2f}" if t.get("evidence_balance") else ""
    ...
    for key, title in [("D_enriched_themes",f"Enriched in {entity}"),
        ...
        if key in ("D3_shared_themes","D4_depleted_themes"):
            with st.expander(f"{title} ({len(items)})"):
                st.dataframe(_tdf(items,True), use_container_width=True, hide_index=True)
        else:
            st.subheader(title)
            st.dataframe(_tdf(items,True), use_container_width=True, hide_index=True)
else:
...
with st.expander(f"Neo4j Browser ({lbl})"): st.code(gr.viz_cypher, language="cypher"); st.markdown("1. localhost:7474  2. Paste  3. Graph view  4. Expand")
