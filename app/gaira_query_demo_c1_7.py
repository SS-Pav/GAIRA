# GAIRA QUERY DEMO — C1.7
# Motif Differential + Signal Stability Layer
...
st.caption(f"Version {VERSION} | Motif differential + cross-study stability | 1,887 evidence rows | 137 sources")
...
with st.sidebar:
    ...
    **Standard:**
    ...
    """)
    st.caption(f"{VERSION}: motif-level differential, per-study stability, cross-study agreement.")
...
if scored.comparator_summary:
    ...
    if cs.scope_fallback: st.warning(f"Scope fallback: {cs.scope_fallback}")

# ── MOTIF DIFFERENTIAL + STABILITY (NEW — PRIMARY SECTION) ─
if scored.motif_differentials:
    st.subheader("Motif Differential + Stability")
    md_rows = []
    for d in scored.motif_differentials[:12]:
        sf = d.subfamily.replace("_"," ") if d.subfamily else d.motif_id
        row = {
            "Motif": sf,
            ...
            "Agree.": f"{d.directional_agreement:.2f}" if d.directional_agreement else "",
        }
        if d.coverage_flag: row["Flag"] = d.coverage_flag.replace("_"," ")
        md_rows.append(row)
    ...
    if ss:
        with st.container():
            ...
            if ss["stable"]: scol1.success(f"STABLE: {', '.join(ss['stable'][:5])}")
            else: scol1.info("No stable signals")
            if ss["mixed"]: scol2.warning(f"MIXED: {', '.join(ss['mixed'][:5])}")
            if ss["unstable"]: scol3.error(f"UNSTABLE: {', '.join(ss['unstable'][:5])}")
            if ss["insufficient"]: scol4.info(f"INSUFFICIENT: {', '.join(ss['insufficient'][:5])}")
...
def _tdf(themes, show_comp=False):
    ...
    for t in themes:
        ...
        if show_comp:
            ...
            r["Direct"] = t.get("direct_evidence",0); r["Sources"] = t.get("source_diversity",0)
        rows.append(r)
    return pd.DataFrame(rows)

if is_comp:
    for key, title in [("D_enriched_themes",f"Enriched Themes ({entity})"),
