...
def load_family_counts() -> tuple[pd.DataFrame, bool]:
    """Per-axis grounding EVIDENCE table (v2: real, generated).

    v1 returned hardcoded per-axis analyte counts (always placeholder). v2
    reads the derived table produced by `tools/build_grounding_evidence.py`
    from the 202-molecule RamanBioLib reference table + the grounding
    registry. Columns:
        axis, axis_short, unique_reference_analytes (int or 'NA'),
    ...
    'NA' (not 0) is used where the legacy 8-axis grounding cannot be resolved
    to a single v11 child axis. Falls back to the curated placeholder only if
    the generated file is missing.
    """
    _gen = cfg.GENERATED_DIR / "per_axis_grounding_evidence.csv"
    # keep_default_na=False so the literal "NA" (not-defensibly-mappable axes)
    # is preserved as text rather than coerced to NaN — the UI must show "NA".
    real = None
    if _exists(_gen):
        try:
            real = pd.read_csv(_gen, keep_default_na=False)
        except Exception:
            real = None
    if real is not None and "unique_reference_analytes" in real.columns:
        # Keep canonical axis order.
        order = {a: i for i, a in enumerate(cfg.BSV_AXES)}
        real = real.assign(_o=real["axis"].map(order)).sort_values("_o").drop(columns="_o")
        # `n_analytes` mirror column for legacy plotting (NA stays NA).
        real["n_analytes"] = real["unique_reference_analytes"]
        return real.reset_index(drop=True), False

    # Fallback: curated placeholder (only if generated file is absent).
    counts = {
...
@st.cache_data(show_spinner=False)
def load_grounding_corpus_summary() -> dict | None:
    """Corpus-level grounding summary (v2 generated). None if absent."""
    import json
    p = cfg.GENERATED_DIR / "grounding_corpus_summary.json"
    if _exists(p):
        try:
