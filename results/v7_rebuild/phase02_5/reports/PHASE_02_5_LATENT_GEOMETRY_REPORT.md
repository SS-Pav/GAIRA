...
discrete islands — but it carries one real top-level bipartition.** The pairwise distance
distribution is unimodal (valley depth 0.003 — no density gap), mean local intrinsic dimension
is 3.9 against an ambient 676, and community conductances run 0.48–0.81. Yet the structure is
not noise: kNN graph modularity is 0.620 against a degree-preserving null of 0.070 ± 0.003
(p < 0.001), and nearest neighbours share a chemistry class 2.8× more often than a label
permutation allows (0.240 vs 0.086, p < 0.0001).

**The one defensible cluster count is K = 2, and it is a hydrophobic/polar split.** Silhouette
(0.551) and Calinski–Harabasz both select K = 2 under four of five linkages, and the split has
bootstrap ARI 0.879. Cluster A (23 motifs) holds 11 of the 13 lipid-class motifs, all eight
protein motifs and both pigment motifs; cluster B (27) holds the saccharides, amino acids,
carboxylates, phosphate and nucleobases, with only 2 lipid motifs. Majority-class purity 0.304
against a label-permutation null of 0.215 ± 0.020, p < 0.0001. This is the same axis PCA found
as its one reproducible component — CH₂ scissoring at 1442 cm⁻¹. **Below K = 2 no cluster count
is defensible**: Davies–Bouldin runs monotonically to the largest K tested while the other two
indices fall away, which is the signature of no further structure.
...
- kNN graph (k = 5): 5 Louvain communities, **modularity 0.620 against a degree-preserving null
  of 0.070 ± 0.003**, p < 0.001 (empirical, 40 rewirings). The community structure is real.
  The z-score, 187, should not be quoted precisely: the null's standard deviation is 0.003, so
  z is numerically unstable between runs (124 → 187 across two runs) while the empirical p
  stays at 0. The p-value is the number to cite.
- Hierarchical clustering: **best silhouette 0.551 at K = 2** (average and complete linkage;
  spectral 0.550, ward 0.502), bootstrap ARI **0.879**.
- Silhouette and Calinski–Harabasz agree on K = 2 under four of five linkages. Davies–Bouldin
  disagrees and picks K = 14–15 — but it does so monotonically, preferring the largest K
  offered, which is its behaviour when there is no structure left to find.

**So there is exactly one defensible cut, and it is at the top.** Below K = 2 the indices
diverge and no count survives. A continuum with one broad bipartition and denser regions inside
it has precisely this signature.
...
| conductance | 0.48–0.81 across regions | boundaries are permeable |
| modularity vs null | 0.620 vs 0.070 ± 0.003, p < 0.001 | denser regions are real |
| cluster indices | agree at K = 2 (silhouette 0.551, bootstrap ARI 0.879), diverge below | **one** defensible cut, at the top |

**Verdict: MIXED — one real bipartition over overlapping continua.** The top-level split is
hydrophobic versus polar (§1); everything inside it is continuous. Region classification:
