...
class LSMRegistry:
    ...
    def component_table(self) -> pd.DataFrame:
        ...
        for r in self.results:
            ...
            rows.append({
                ...
                "mean_purity": round(float(np.mean([m.purity for m in kept])), 4) if kept else None,
                "mean_stability": round(float(np.mean([m.stability for m in kept])), 4) if kept else None,
    ...
    def rejection_table(self) -> pd.DataFrame:
        ...
        return (pd.DataFrame(rows).sort_values(["parent_component", "motif_id"])
                .reset_index(drop=True) if rows else
                pd.DataFrame(columns=["motif_id", "parent_component", "n_analytes",
    ...
    def check_integrity(self) -> list[str]:
        ...
        for m in self.motifs:
            if m.parent_component not in comps:
                bad.append(f"{m.motif_id}: parent component {m.parent_component} not in results")
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "motifs_per_component_max": int(comp.n_retained_motifs.max()) if len(comp) else 0,
            "mean_purity": round(float(np.mean([m.purity for m in kept])), 4) if kept else None,
            "mean_stability": round(float(np.mean([m.stability for m in kept])), 4) if kept else None,
            "mean_coverage_analytes": round(float(np.mean([m.coverage_analytes for m in kept])), 4) if kept else None,
