"""GAIRA V7 — validation of the Local Spectral Motif layer.
...
Two things make that question answerable rather than rhetorical:
  ...
  * chemistry is used ONLY here, in evaluation. No class label touched band definition,
    ...
    cannot be circular.
  * every alignment claim is measured against a **permutation null** with a chance-corrected
    statistic (adjusted mutual information). A raw purity number would rise simply by
    cutting a set into more pieces; AMI would not.
"""
...
def chemical_alignment(results: list[dict], fine_of: dict, broad_of: dict,
    ...
    """Per component: do the motif memberships align with chemistry beyond chance?"""
    ...
    for r in results:
        ...
        if len(kept) < 2:
            rows.append({"component": r["component"], "status": r["status"],
                         ...
                         "ari_fine": None, "ami_broad": None, "p_permutation": None,
...
def ambiguity_resolution(results: list[dict], fine_of: dict) -> pd.DataFrame:
    ...
    `component_dominant_share` is the purity of the component taken whole. `best_motif_purity`
    is the purity of its purest retained motif. The gap is the resolution the motif layer
    actually delivers for that component.
    """
    rows = []
    for r in results:
        if r["status"] == "NOT_ANALYSABLE":
        ...
        wmean = (sum(m.purity * m.n_analytes for m in kept) /
                 sum(m.n_analytes for m in kept)) if kept else float("nan")
        rows.append({
...
def redundancy_matrix(motifs) -> pd.DataFrame:
    ...
    return pd.DataFrame(np.round(C, 4), index=ids, columns=ids)
...
def reproducibility(discover_fn, subsets: dict[str, dict], reference_results: list[dict],
                    ) -> pd.DataFrame:
    """Re-run discovery on data subsets and measure agreement with the full-corpus run.

    Agreement is the adjusted Rand index between motif memberships restricted to the
    analytes present in both runs — a chance-corrected measure, so a component that
    trivially yields one motif does not score as perfect reproducibility.
    """
    ref = {}
    ...
    for name, kwargs in subsets.items():
        ...
        for r in got:
            ...
            kept = [m for m in r["motifs"] if m.retained]
            if len(kept) < 2:
                rows.append({"subset": name, "component": k, "n_shared": 0,
                             "ari": None, "note": "subset run gave <2 motifs"})
                continue
...
def determinism_check(discover_fn, kwargs: dict, n_runs: int = 3) -> dict:
    ...
    for _ in range(n_runs):
        ...
        kept = sorted([m for r in res for m in r["motifs"] if m.retained],
                      key=lambda m: m.motif_id)
        import hashlib
        h = hashlib.sha256()
        for m in kept:
            h.update(m.motif_id.encode())
            h.update(np.ascontiguousarray(m.spectrum, dtype=np.float64).tobytes())
        sigs.append(h.hexdigest()[:32])
