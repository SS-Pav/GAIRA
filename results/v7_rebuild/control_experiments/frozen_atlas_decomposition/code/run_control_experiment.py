#!/usr/bin/env python3
"""GAIRA V7 — Phase 01 orchestrator: Atlas Component Substructure discovery (Strategy A).

Deterministic end to end. Reads the FROZEN atlas and the FROZEN Phase 00 outputs; writes
only under results/v7_rebuild/control_experiments/frozen_atlas_decomposition/. Never touches assets/, results/v5_rebuild/,
results/v6_rebuild/ or results/v7_rebuild/phase00/.

    python results/v7_rebuild/control_experiments/frozen_atlas_decomposition/code/run_phase01.py [--data-root PATH]
"""
...
PHASE, PHASE_NAME = "control", "Frozen-atlas decomposition (CONTROL EXPERIMENT — not canonical V7)"
TABLES, FIGURES = CTRL / "tables", CTRL / "figures"
REPORTS, VALID = CTRL / "reports", CTRL / "validation"
LOGS, ARTIFACTS = CTRL / "logs", CTRL / "artifacts"
...
def main() -> int:
    ...
    from gaira.v7.atlas_decomposition import motif as MO
    cons_rows = []
    for cname, fn in (("discriminative (selected)", MO.build_motif_spectrum),
        ...
        res = DIS.discover_all(H, grid, Xa, Wa, ids, fine_of, broad_of, sources_of,
                               spectra_of)
        rr = ACSRegistry(res, fp_before, DIS.DISCOVERY_VERSION, {})
    ...
    reg = ACSRegistry(results, fp_before, DIS.DISCOVERY_VERSION, config)
    summ = reg.summary()
    ...
    for n, h in man["files"].items():
        outputs.append({"artifact_id": n, "path": str((ARTIFACTS / n).relative_to(REPO)),
                        "sha256": h})
    outputs.append({"artifact_id": "acs_manifest_v1.json",
                    "path": str((ARTIFACTS / "acs_manifest_v1.json").relative_to(REPO)),
                    "sha256": P.sha256_file(ARTIFACTS / "acs_manifest_v1.json")})
    outputs.append(wtab(reg.motif_table(), "acs_registry_v1.csv"))
    outputs.append(wtab(reg.component_table(), "acs_components_v1.csv"))
    outputs.append(wtab(reg.rejection_table(), "acs_rejections_v1.csv"))
    ...
    manifest = {
        ...
        "code": {"git_sha": git["git_sha"], "branch": git["branch"], "dirty": git["dirty"],
                 "entry_point": "results/v7_rebuild/control_experiments/frozen_atlas_decomposition/code/run_phase01.py",
                 "package": "src/gaira/v7/lsm/"},
        "environment": env, "outputs": outputs, "gates": gates, "decisions": DECISIONS,
    }
    outputs.append(wjson(manifest, "control_manifest_v1.json"))

    state = {
        "schema": "gaira_v7_control_experiment_state_v1",
        "classification": "CONTROL EXPERIMENT — NOT CANONICAL V7",
        "reclassified_by": "GAIRA_v7_rebuild/context/ARCHITECTURE_COMPLIANCE_AUDIT.md",
        "reason": ("Decomposes the FROZEN V5 atlas. The approved V7 architecture derives "
                   "Local Spectral Motifs from an independent class-local NMF over balanced "
                   "references — a different object entirely."),
        "object_name": "Atlas Component Substructure (ACS)",
        "role": ("no-fitting baseline: how much chemical resolution is recoverable from the "
                 "existing atlas with zero learned parameters. The canonical class-local "
                 "Phase 01 must beat this."),
    ...
    wjson(state, "CONTROL_STATE.json", where=CTRL)
    (LOGS / "control_run.log").write_text("\n".join(LOG) + "\n")
