> # ⚠ CONTROL EXPERIMENT — NOT CANONICAL V7
>
> **This is not the V7 Phase 01.** It decomposes the **frozen V5 atlas components**; the
> approved V7 architecture derives Atlas Component Substructures from an **independent class-local
> NMF over balanced references**. See
> [`ARCHITECTURE_COMPLIANCE_AUDIT.md`](../../../../GAIRA_v7_rebuild/context/ARCHITECTURE_COMPLIANCE_AUDIT.md).
>
> **What this experiment is:** the no-fitting baseline — how much chemical resolution is
> recoverable from the existing atlas with zero learned parameters. That is the control the
> canonical class-local rebuild must beat, and it is retained in full for exactly that purpose.
>
> **Object renamed.** Its outputs are **Atlas Component Substructures (ACS)**, not LSMs. The
> term `ACS` is reserved by `TERMINOLOGY_AND_DEFINITIONS.md` for a row of `H_c`.
>
> Results, figures and validation are unchanged and unretracted. Only the label is corrected.

# GAIRA V7 — Control Experiment
## Frozen-atlas component decomposition (formerly mislabelled "Phase 01")

**Branch** `gaira-v7-rebuild` · **Status** COMPLETE · **Control gates** 9/9 PASS (control-experiment gates, NOT V7 phase gates) · **Tests** 47 passed
...
| | |
|---|---:|
| Retained Atlas Component Substructures | **98** (of 128 candidates; 30 rejected with reasons) |
...
The V7 architecture documents and this phase use the term "ACS" for **two different objects**,
...
2. If the class-local-NMF ACS is still wanted later, it needs a different name, or this
...
|---|---|
| `motif.py` | the `ACS` and `Band` objects, structural invariants, motif-spectrum construction |
...
1. **This is not the architecture documents' ACS** (§2). It decomposes frozen components
...
1. **Resolve the naming collision** (§2). Recommendation: keep "ACS" for this
